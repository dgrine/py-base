...
def formatting_public_props():
    return (name for name in dir(formatting) if not name.startswith('_'))

def get_color_string(type, string):
    end = formatting.ENDC
    if type == formatting.WHITE:
        end = ''
...
class Texttable:
    ...
    def _len_cell(self, cell):
        ...
        for attr in formatting_public_props():
            cell = cell.replace(getattr(formatting, attr), '').replace(formatting.ENDC,'')
        ...
        for line in cell_lines:
            ...
            for part, i in zip(parts, list(range(1, len(parts) + 1))):
                for attr in formatting_public_props():
                    part = part.replace(getattr(formatting, attr), '')
    ...
    def _draw_line(self, line, isheader=False):
        ...
        for i in range(len(line[0])):
            ...
            for cell, width, align in zip(line, self._width, self._align):
                length += 1
                cell_line = cell[i]
                lost_color = formatting.WHITE
                original_cell = cell_line
                for attr in formatting_public_props():
                    cell_line = cell_line.replace(
                        getattr(formatting, attr), '').replace(formatting.ENDC,''
                    )
                    if cell_line.replace(formatting.ENDC,'') != original_cell.replace(
                            formatting.ENDC,'') and attr != 'ENDC':
                        if not lost_color:
                            lost_color = attr
                fill = width - len(cell_line)
                try:
                    cell_line = get_color_string(
                        getattr(formatting, lost_color),cell_line
                    )
    ...
    def _splitit(self, line, isheader):
        ...
        for cell, width in zip(line, self._width):
            ...
            for attr in formatting_public_props():
                cell = cell.replace(
                    getattr(formatting, attr), '').replace(formatting.ENDC,'')
                if cell.replace(formatting.ENDC,'') != original_cell.replace(
                        formatting.ENDC,'') and attr != 'ENDC':
                    if not lost_color:
                        lost_color = attr
            for c in cell.split('\n'):
                ...
                try:
                    array.extend(
                        [get_color_string(
                            getattr(formatting, lost_color),x
...
if __name__ == '__main__':
    ...
    table.set_cols_align(["l", "r", "c"])
    table.set_cols_valign(["t", "m", "b"])
    table.add_rows([ [get_color_string(formatting.GREEN, "Name Of Person"), "Age", "Nickname"],
                     ["Mr\nXavier\nHuon", 32, "Xav'"],
                     [get_color_string(formatting.BLUE,"Mr\nBaptiste\nClement"), 1, get_color_string(formatting.RED,"Baby")] ])
    print(table.draw() + "\n")
